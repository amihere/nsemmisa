  ...
  <section class="py-8">
    <article>
      ...
      {#if post.date || post.author || post.tags}
        <aside
          class="text-metadata flex flex-row flex-wrap gap-x-6 px-4 py-3 md:px-8"
        >
          {#if post.date}
            <a class="flex flex-row gap-2 py-1" href="/{post.id}">
              <span class="flex flex-row">
                {#if post.pinned}<PinIcon />{:else}<CalendarIcon />{/if}
              </span>
          ...
          {#if post.author}
            <a class="flex flex-row gap-2 py-1" href="/author/{post.authorId}">
              <span class="flex flex-row"><PenToolIcon /></span>
              <span class="underline">{post.author} </span>
            </a>
          {/if}
            ...
            <div class="flex flex-row items-start gap-2">
              ...
              <div>
                {#each post.tags as tag, i}
                  {#if i},
                  {/if}
                  <a class="inline-block py-1 underline" href="/tag/{tag[0]}">
                    {tag[1]}
                  </a>
                {/each}
              </div>
